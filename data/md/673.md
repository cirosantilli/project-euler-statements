...
The downloadable text files [beds.txt](../documents/0673_beds.txt) and [desks.txt](../documents/0673_desks.txt) contain pairings for $n=500$. Each pairing is written on its own line, with the student numbers of the two roommates (or desk partners) separated with a comma. For example, the desk pairing in the $n=4$ example above would be represented in this file format as:
