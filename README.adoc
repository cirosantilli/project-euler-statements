...
* link:spec.yaml[]: Python-like language independent definition of our chosen inputs and output which each solver in each language must match, sample:
+
----
1:
  args:
    n:
      type: int
  return: Sum of all the multiples of 3 or 5 or below `n`.
3:
  args:
    n:
      type: int
...
* link:data/lean/[]: formal <<lean>> statements of the problems, i.e. a naive implementation `naive` and proofs that the implementation is defined for all inputs
* link:data/py/[]: naive solutions for the problems in Python
...
* link:data/lean/ProjectEulerStatements/P47.lean[]: TODO we need to prove that this always exists for every n. Currently uses some random limit. Might be an open result.
* link:data/lean/ProjectEulerStatements/P49.lean[]: yet another ugly digits one. Generalized as much as possible, we'll see.
* link:data/lean/ProjectEulerStatements/P51.lean[]
* link:data/lean/ProjectEulerStatements/P52.lean[]
* link:data/lean/ProjectEulerStatements/P54.lean[]
* 55-58
...
=== Existence proof conventions

The approach we take most of the time for Project Euler problems is to define a function naive that matches the problem for any integer.

Our statements should always attempt to prove that such functions are actually defined for all values.

However, some problems are not known to have such a valid solution for every possible input, pending the resolution of ope mathematical problems.

In such cases, our convention is as follows:

* `sorry`: a TODO, we need to research more to make sure the problem is actually open
* `axiom`: we have researched enough and have decided that it is surely or likely open, so we will just assume it is true
